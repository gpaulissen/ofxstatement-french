...
class Parser(BaseStatementParser):
    ...
    def add_cache(self,
        ...
        """Set the transaction"""
        # Sometimes payee is memo or otherwise: forget them
        key: TransactionKey = Parser.transaction_key(account_id,
                                                     check_no,
                                                     dtposted,
                                                     amount)
        data: TransactionData = Parser.transaction_data(id,
                                                        payee,
                                                        memo)
        ...
        msg: str = 'file: %s; key: %r; data: %r' % (ofx_file, key, data)

        assert id not in self.unique_id_sets[account_id] or\
            self.cache.get(key, data) == data,\
            "Found " + str(self.cache.get(key, data)) +\
            " while adding " + msg
